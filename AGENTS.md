...
### Scene Flow
```
PreloadScene → MainMenuScene → Level1 → Victory → Shop → Level2 → Victory → Shop → Level3 → Victory → Shop → Level4 → Victory → Shop → Level5 (+ HUD overlay) → portal → Victory/GameOver → MainMenu/retry
```

Level 5 ends differently: key #5 opens the 5-key portal in the arena (`def.portal`), and walking into it starts Victory. When Level 6 exists, flip Level 5's `nextSceneKey`; the portal needs no other change.
...
- **`src/levels.ts`** — data-driven level registry (Phaser-free; vitest imports it). Each `LevelDef` holds world size, parallax/texture keys, platforms/stairs, zombie spawns (with optional explicit `y` for elevated spawns), a `BossDef` (stats, tint, charge/leap flags, optional minion `summon`), and boss-arena geometry. `TRAIN` exports Level 3's train layout. Optional blocks: `water` (Level 4), `candy` (Level 5: marshmallow pads + candy enemy spawns), and `portal`. `BossDef` is a union on `kind`: `'walker' | 'kraken' | 'worm'`. Layout gotchas are enforced by `levels.test.ts` invariants — new level data gets validated for free.
- **`src/config.ts`** — every gameplay tunable (player physics, combat, zombie/boss AI, `ZOMBIE.variants` tint/scale/stat table — power-monster variants add `powerUp` + `bakeColor`). Also the shop/power-up data tables: `SWORDS` (tier list), `CONSUMABLES` (potion/shield/life cost+cap), `SHOP`, `POWERUPS` (per-buff name/duration/color), `BUFF` (flight/giant/damage tunables). Level blocks: `WATER`/`FISH`/`EEL` (Level 4) and `CANDY` (Level 5: bounce velocities, pad size, sour candy cap, gummy/gumball/chocolate stats). Tune here or in the level def, not in entity code.
...
- **Marshmallow pads & the Worm King (Level 5)**: pads are a static group; the pad collider calls `player.bounceOnPad()` on a landing from above (slam = higher pogo bounce). The scene owns the sour candy pocket (pure `core/sourCandy.ts`) and hands the `Worm` two hooks: `spendCandy` (a sword hit on the open mouth feeds him → dizzy) and `knockLoose` (a hit when he is not dizzy drops a candy, on a cooldown). Gumball kills also drop a candy.
...
- Levels 1-5 fully playable end-to-end with Victory chaining, level-aware GameOver retry, and MainMenu replay (number keys):
  ...
  - **Level 4 — The Zombified Lake** (co-designed with Henry 2026-07-12): underwater **swim mode** (↑ thrust, reduced gravity, torpedo dash, dolphin surface pop; land jumps/slam gated off), a ~30s **air meter** with drowning + surface/vent refill, destructible **scuba gear** (5-hit crack stages), water enemies (drowned float-chasers, darting fish schools, ambush eels — all `Hittable`), and the **SUNKEN BEAST kraken** boss (toxic green with 4 eyes, 6 when enraged — Henry and Wes 2026-09-22; swims a figure-8 toward the player, tuned by `KRAKEN_SWIM` in config.ts and driven by pure `stepSwim`; tentacle-guarded head → timed damage window at half swim speed → regrow; the active guard wears a baked glow texture and pulses; a guarded head hit CLANKs with a "CHOP THE GLOWING TENTACLE!" hint; guard chops also come off the boss health, so the bar moves on every hit; aimed laser bubbles; 50% enrage 3-spread + baked red-eye frames). Gated on `def.water`; pure cores `air.ts`/`kraken.ts`/`water.ts`. The walker boss now runs through a shared `BossEncounter` interface alongside the Kraken
  - **Level 5 — The Sugar Rush Zone** (designed by Wes, age 5, 2026-09-22): a rotten candy world with **marshmallow bounce pads**, **gummy bear zombies** that split into 2 cubs, **gumball zombies** that shake and roll (and drop sour candy), **chocolate zombies** that melt into a puddle and reform behind you, and the **GUMMY WORM KING** (digs under the arena, shakes the ground, pops up to chomp; a sour candy in his open mouth makes him dizzy = the damage window; 50% enrage summons gummy cubs). Key #5 opens the **5-key portal**. Gated on `def.candy`; pure cores `sourCandy.ts`/`worm.ts`
...
- **Power monsters**: 4 baked-color zombie variants, named by Henry 2026-07-12 — SKY SCREECHER (`vulture`), FURYFANG (`rage`), TITAN ZOMBIE (`titan`), GEM GUARDIAN (`crystal`); names show as float text on kill. They spawn across Levels 1-5 (Gem Guardian + Titan collectible underwater on L4); kills drop buff orbs granting flight / mega damage / giant mode / invincibility with HUD countdowns; orbs magnetize to the player when the boss triggers
...
- Verified by automated browser playtest: full 4-level progression incl. shop purchases/persistence, power-monster orb→buff→HUD flow, revive edge cases, the L4 swim/air/scuba/kraken flow (Canvas) + lights/tints/FX + enraged red-eye pass (headed WebGL), and the L5 bounce/split/roll/melt/feed/dizzy/key/portal flow (Canvas) + candy lighting pass (WebGL); 205 vitest invariants

**Next milestones:** play Level 5 with Wes and tune it (bear looks, bounce height, Worm King HP — a perfect player needs only 2 feeds); Level 4 feel tuning with Henry (air budget vs. water depth, murk/shaft readability, drowning-cue urgency); **Level 6 — The Dark Underworld** with Henry's Zombie Taco Truck boss (was Level 5 in the first design — Wes's candy level took slot 5 on 2026-09-22), then Level 7, the final Giant Brain Behemoth Titan Zombie. Level 6 needs a reward decision: `GameState.keys` has only 5 slots
...
- **Scene `update()` and Arcade physics steps do not run 1:1** (headless runs showed two updates per physics step). Never integrate distance as `speed × delta` for gameplay rules — measure real movement (`Math.abs(x - lastX)`), as the Gumball roll does
- **A boss arena narrower than the 960px screen shows empty space past the world end** (the locked camera clamps to the arena's left edge). Level 5's arena is 960px wide and a test enforces it for candy levels; Levels 1-4 use 540-600px arenas and likely show the same gap (not yet checked in play)
